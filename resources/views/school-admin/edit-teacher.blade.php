...
@section('page-heading','Teachers / Edit ('.$teacher->name.')')
@section('school-admin-content')
    @php
        $school = \App\School::findOrFail(getCurrentSchoolId());
    @endphp
    <h2 class="h2 text-center">Edit Teacher</h2>
    <div class="d-block container">
        <a href="{{ route('school-admin-show-teacher',['id'=>$teacher->id]) }}"
           class="btn btn-primary bg-gradient-primary">View</a>
        <input type="button" class="btn btn-danger" value="Delete" name="delete" id="delete_teacher_btn_{{ $teacher->id }}">
        <form action="{{ route('school-admin-destroy-teacher',['id' => $teacher->id]) }}"
            ...
            <input type="hidden" value="{{ $teacher->id }}" name="id" id="teacher_id_{{ $teacher->id }}">
        </form>
    </div>
    @push('js')
        @php
            ...
            $title .= "<h6 class=\"small d-inline-block text-truncate w-75\"> User : ".$teacher->User->email." </h6>";
            $title .= "<h6 class=\"small d-inline-block text-truncate w-75\"> School Name : ".$teacher->School->name."</h6>";
    ...
    @include('snippets.change-image',[
        ...
        'current_image_url' => getPassportPhotoImageUrl($teacher->passport_photo),
        'image_upload_url' => route('school-admin-update-teacher-passport-photo',['id' => $teacher->id]),
        'redirect_url' => null,
    ])
{{-- Edit Form for Teacher--}}
    <form action="{{ route("school-admin-update-teacher-text-data",['id'=>$teacher->id]) }}"
          method="POST"
          class="w-75 col-xxl-6 col-xl-6 col-lg-6 m-auto"
          id="save_teacher_form">
        @method('PUT')
        @csrf
            ...
            <select name="user" id="user"
                ...
                @foreach(\App\User::orderBy('email')->get() as $user)
                    @if(strcasecmp(old('user'),$user->id) == 0)
                        ...
                        @endphp
                    @else
                        @if(strcasecmp($user->id,$teacher->user_id)==0)
                            @php
                                $selected = 'selected = "selected"';
                            @endphp
                        @else
            ...
            <select name="gender" id="gender"
                ...
                @foreach($gender_contents as $gender_value => $gender_content)
                    @if(strcasecmp(old('gender'),$gender_value) == 0)
                        ...
                        @endphp
                    @else
                        @if(strcasecmp($gender_value,$teacher->gender)==0)
                            @php
                                $selected = 'selected = "selected"';
                            @endphp
                        @else
        ...
        <div class="form-group">
            ...
            <input type="text" name="district"
                   ...
                   min="2" max="255"
                   value="{{ old('district') ?? $teacher->district }}">
            @error('district')
            <span class="invalid-feedback" role="alert">
                        <strong>{{ $message ?? $teacher->district }}</strong>
                    </span>
            @enderror
        </div>
        <div class="form-group">
            ...
            <select name="country" id="country"
                ...
                @foreach(config("utilities.countries") as $country_code => $country)
                    @if(strcasecmp(old('country'),$country) == 0)
                        ...
                        @endphp
                    @else
                        @if(strcasecmp($country,$teacher->country)==0)
                            @php
                                $selected = 'selected = "selected"';
                            @endphp
                        @else
        ...
        <div class="form-group">
            ...
            <input type="text" name="phone1"
                   ...
                   required min="2" max="255"
                   value="{{ old('phone1') ?? $teacher->phone1 }}">
            @error('phone1')
            <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
        ...
        <div class="form-group">
            ...
            <input type="text" name="phone2"
                   ...
                   min="2" max="255"
                   value="{{ old('phone2') ?? $teacher->phone2 }}">
            @error('phone2')
            <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
        ...
        <input type="button" value="Save Teacher" id="save_teacher_btn"
               class="btn btn-lg bg-gradient-primary float-right text-white">
    </form>
@endsection
@push('js')
    @php
        $title = "<h5 class=\"h5 font-weight-bolder\">Please Confirm your Password Before Saving the Teacher</h5>";
        $title .= "<h6>ID: ".$teacher->id."</h6>"."<h6>Name: ".$teacher->name."</h6>";
        $title .= "<h6>School Name: ".$school->name."</h6>";
        $res = passwordConfirmationBoxScript("#save_teacher_btn","#save_teacher_form","Password",$title);
        echo($res);
    @endphp
@endpush
