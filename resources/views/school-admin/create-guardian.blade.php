...
@section('page-heading','Guardians / Create')
@section('school-admin-content')
    @php
        $school = \App\School::findOrFail(getCurrentSchoolId());
    @endphp
    <h2 class="h2 text-center">Add/Create/Register Guardians</h2>
    <form action="{{ route("school-admin-store-guardian") }}"
          method="POST"
          class="w-75 col-xxl-6 col-xl-6 col-lg-6 m-auto"
          id="create_guardian_form">
        @csrf
        ...
        <div class="form-group">
            <label for="student">Student <span class="text-danger"> ( Required) </span></label>
            <select name="student" id="student"
                    class="form-control @error('student') is-invalid @enderror"
                    required>
                <option value="">-- Select Student --</option>
                @foreach(\App\School::findOrFail(getCurrentSchoolId())->Students->sortBy('name') as $student)
                    @php
                        $old_student = old('student');
                    @endphp
                    @if(isset($old_student))
                        @if(strcasecmp(old('student'),$student->id) == 0)
                            @php
                                $selected = 'selected = "selected"';
                            @endphp
                        @else
                    ...
                    @else
                        @if(isset($_GET['student_id']))
                            @if(strcasecmp($_GET['student_id'],$student->id)==0)
                                @php
                                    $selected = 'selected = "selected"';
                                @endphp
                            @else
                    ...
                    <option
                        ...
                        loading='lazy'
                        style='height:24px;weight:24px;' class='mr-3'>{{ $student->name }}
                            <sub>Student ID: {{ $student->id }}</sub>
                            </div>">
                        {{ $student->name }}
                    </option>
                @endforeach
        ...
        <input type="button" value="Add Guardian" id="add_guardian_btn"
               class="btn btn-lg bg-gradient-primary float-right text-white">
    </form>
@endsection
@push('js')
    @php
        $title = "<h5 class=\"h5 font-weight-bolder\">Please Confirm your Password Before Adding/Creating/Registering the Guardian for School</h5>";
        $title .= "<h6>School Name: ".$school->name."</h6>";
        $res = passwordConfirmationBoxScript("#add_guardian_btn","#create_guardian_form","Password",$title);
        echo($res);
    @endphp
@endpush
